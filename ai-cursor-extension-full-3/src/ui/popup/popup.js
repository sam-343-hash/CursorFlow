  ...
  chrome.runtime.sendMessage({ type: 'GET_PROVIDER_STATUS' }, (response) => {
    ...
    if (names.length === 0) {
      ...
      providerStatusEl.innerHTML = `
        ⚠️ No AI provider configured - using basic keyword matching only, which cannot understand intent.
        <div class="quick-key-row">
          <input type="password" id="quickGeminiKey" placeholder="Paste a free Gemini key">
          <button id="quickSaveKey" class="mini-btn">Save</button>
        </div>
        <a id="setupProviderLink">Or open full Settings</a>
      `;
      const link = document.getElementById('setupProviderLink');
      if (link) link.addEventListener('click', () => chrome.runtime.openOptionsPage());

      const quickSaveBtn = document.getElementById('quickSaveKey');
      const quickKeyInput = document.getElementById('quickGeminiKey');
      if (quickSaveBtn && quickKeyInput) {
        quickSaveBtn.addEventListener('click', () => {
          const key = quickKeyInput.value.trim();
          if (!key) return;
          chrome.storage.local.set({ geminiApiKey: key }, () => {
            checkProviderStatus(); // re-check immediately so the banner updates without reopening the popup
          });
        });
        quickKeyInput.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') quickSaveBtn.click();
        });
      }
    } else {
