...
## Cross-page tasks (navigation)

A goal can now span more than one page - e.g. "go to github.com and check my notifications." When the AI decides the next step requires a different website, it can NAVIGATE the current tab or OPEN a new one, and a background-side mission record makes the goal automatically continue once that page finishes loading, even though the original Planner instance (which is tied to the page it started on) is gone the moment navigation happens.

**Safety boundary:** the AI may only navigate to a site the user explicitly named in their own goal text (checked in code, in `navigationActions.js`, not just requested via the prompt) - it cannot autonomously choose which website to visit for an open-ended goal like "find me the best flight." That kind of independent site selection is a deliberately separate, more heavily guarded capability, not included here.

This required adding the `tabs` permission (previously the extension only used `activeTab`, which only covers direct, immediate user-gesture-triggered actions - an AI's own navigation decision, made asynchronously after reasoning about a goal, does not qualify as that kind of direct gesture).
