...
(function () {
  ...
  class Planner {
    ...
    constructor(deps) {
      ...
      this.navigator = deps.navigator || null; // optional: async (action, url) => void
    ...
    async run(goal, options) {
      ...
      while (true) {
        ...
        // --- NAVIGATION: a whole-tab action, no element index involved ---
        // This is a deliberate, natural stopping point for THIS page-level
        // run: navigating destroys the current page (and this content
        // script's execution context) entirely. A higher-level mission
        // controller (see src/mission/) is responsible for noticing the
        // navigation completed and starting a fresh Planner run on the
        // new page toward the same goal - this Planner instance's job
        // ends the moment navigation is handed off successfully.
        if (ensembleResult.action === 'navigate' || ensembleResult.action === 'open_tab') {
          this.state = STATES.NAVIGATING;
          emit(STATES.NAVIGATING, {
            message: `Navigating to ${ensembleResult.url} (${ensembleResult.note || ''})`,
            action: ensembleResult.action,
            url: ensembleResult.url,
          });

          if (!this.navigator) {
            this.state = STATES.FAILED;
            const reason = 'The AI wants to navigate to a URL, but no navigator was configured for this Planner.';
            emit(STATES.FAILED, reason);
            return { status: STATES.FAILED, reason, steps };
          }

          try {
            await this.navigator(ensembleResult.action, ensembleResult.url);
          } catch (err) {
            this.state = STATES.FAILED;
            const reason = `Navigation failed: ${(err && err.message) || err}`;
            emit(STATES.FAILED, reason);
            return { status: STATES.FAILED, reason, steps };
          }

          this.loopGuard.recordStep();
          steps.push({ stepNumber: this.loopGuard.stepCount, label: null, success: true, reason: `Navigated to ${ensembleResult.url}` });
          return { status: STATES.NAVIGATING, reason: `Navigating to ${ensembleResult.url} to continue the task.`, steps };
        }
        ...
        // --- ACT ---------------------------------------------------------
        // Defaults to 'click' when the ensemble result doesn't specify an
        // action at all - this keeps every existing caller (including
        // simple test doubles that only return {index, note}) working
        // unchanged; 'type' is an additive capability, not a breaking one.
        const actionType = ensembleResult.action || 'click';
        this.state = STATES.ACTING;

        if (actionType === 'type') {
          emit(STATES.ACTING, {
            message: `Typing "${ensembleResult.value}" into: "${targetMeta.label}" (${ensembleResult.note || ''})`,
            elementIndex: ensembleResult.index,
            label: targetMeta.label,
            sensitive: false,
            actionType: 'type',
        ...
        let actionOutcome;
        try {
          if (actionType === 'type' && ensembleResult.value) {
            actionOutcome = await this.actionQueue.enqueue(
              () => this.elementActions.typeText(targetElement, ensembleResult.value),
              {
                label: targetMeta.label,
