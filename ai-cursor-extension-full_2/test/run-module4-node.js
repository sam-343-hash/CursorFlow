...
async function main() {
  ...
  // --- Planner: executes navigation when the ensemble requests it -----------
  let navigatedTo = null;
  const navigatingPlanner = new AICursor.Planner(buildFakeDeps({
    runEnsemble: async () => ({ action: 'navigate', url: 'https://github.com', note: 'goal named this site' }),
    navigator: async (action, url) => { navigatedTo = { action, url }; },
  }));
  const navPlannerResult = await navigatingPlanner.run('go to github.com and check notifications');
  check(
    'Planner: calls the injected navigator and stops with NAVIGATING status',
    navPlannerResult.status === S.NAVIGATING && navigatedTo && navigatedTo.url === 'https://github.com',
    JSON.stringify({ navPlannerResult, navigatedTo })
  );
  check('Planner: NAVIGATING is a terminal state', AICursor.PlannerStates.isTerminal(S.NAVIGATING) === true);

  // --- Planner: fails clearly if navigation is requested but no navigator configured ---
  const noNavigatorPlanner = new AICursor.Planner(buildFakeDeps({
    runEnsemble: async () => ({ action: 'navigate', url: 'https://github.com', note: 'x' }),
  }));
  const noNavigatorResult = await noNavigatorPlanner.run('go to github.com');
  check(
    'Planner: fails clearly (not crash) when navigation is requested with no navigator configured',
    noNavigatorResult.status === S.FAILED && noNavigatorResult.reason.includes('navigator'),
    JSON.stringify(noNavigatorResult)
  );

  // --- Planner: navigator throwing is treated as a clean failure, not a crash ---
  const failingNavigatorPlanner = new AICursor.Planner(buildFakeDeps({
    runEnsemble: async () => ({ action: 'navigate', url: 'https://github.com', note: 'x' }),
    navigator: async () => { throw new Error('tab was closed'); },
  }));
  const failingNavResult = await failingNavigatorPlanner.run('go to github.com');
  check(
    'Planner: a throwing navigator produces a clean FAILED result, not an unhandled crash',
    failingNavResult.status === S.FAILED && failingNavResult.reason.includes('tab was closed'),
    JSON.stringify(failingNavResult)
  );
